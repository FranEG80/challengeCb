...
///////////////timeStop: '',
  ...
  keys: {
    ...
    removeToGroup: ["no voy", "quitame", "desapuntame"]
  },
  hystorical: {},
  users: userParseo,
  grouping: {
...
/////////////// },
  ...
  sendMessage: async (channel, event, message, res) => {
    res.sendStatus(200);
    const users = [ 'UG51B0XPD', 'UNQPY9RQC', 'UP2RYT38U' ].join(',')
    try {
...
//////console.log('------', channelInfo)
      console.log(`Successfully send message ${result.ts} in conversation ${channelInfo.channel.name}`);
    } catch (error) {
  ...
  isInTheKeys: (inputKey, mentioned, message_recived) => {
  ...
  initGroup: (channel) => {
    ...
    if (this.BOT.grouping[channel] !== undefined && this.BOT.grouping[channel].status ) {
      return `Ya hemos iniciado los grupos! ${msg}`
    } else {
      ...
      return `Ok! vamos a buscar los grupos, ${msg}`
    }
  },
  ...
  stoppingGroup: (channel, res) => {
    if (this.BOT.grouping[channel] !== undefined && this.BOT.grouping[channel].status === true) {
      ...
      this.BOT.creatingGroups(channel, this.BOT.grouping[channel].users, res)
    } else {
      this.BOT.sendMessage(channel, `No hemos empezado todavía, si quieres escribe *gordify start*`, res) 
    }
  },

  addingGroup: (channel, user) => {

    if(this.BOT.grouping[channel] === undefined || !this.BOT.grouping[channel].status) {
      return `<@${user}> no estamos organizando los grupos! prueba a inciar la búsqueda con *gordify start*`
...
//////////////// Create parseo users
      return `Ok <@${user}> te apunto a la lista de los grupos!! :smile:` 
    } else {
      return `Ey!!! ya contaba contigo <@${user}> :smile::smile::smile:`
    }

  },


  isMentioned: type =>  type === "app_mention",

  creatingGroups: (channel, users, res) => {
...
/////// elegir leader sin repetir --------------------------
    ...
    this.BOT.sendMessage(channel, "blocks", template, res)
